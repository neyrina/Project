...
public class TestingPage extends BasePage {
    ...
    public void chooseCourseCard() {
        ...
        for (int i = 1; i <= courses.size(); i++) {
            ...
            switch (courseUrl) {
                case "https://otus.ru/lessons/java-qa-pro/?int_source=courses_catalog&int_term=testing"://Т.к локаторы на страницах отличаются, пришлось наплодить кейсов
                case "https://otus.ru/lessons/avtomatizaciya-web-testirovaniya/?int_source=courses_catalog&int_term=testing"://решила оставить ссылки, т.к удобнее сразу кликать на нее для проверки. А с енам что-то не сложилось в этот раз
                    Assert.assertFalse("Не указан заголовок курса " + courseUrl, driver.findElement(By.xpath("//div[@class='course-header2__title']")).getText().isEmpty());
                    Assert.assertFalse("Не указано описание курса " + courseUrl, driver.findElement(By.xpath("//div[@class='course-header2__admin-text']")).getText().isEmpty());
                    commonCheck();
                    break;
                ...
                case "https://otus.ru/lessons/qajs/?int_source=courses_catalog&int_term=testing":
                    Assert.assertFalse("Не указан заголовок курса " + courseUrl, driver.findElement(By.xpath("//div[@class='course-header2__title']")).getText().isEmpty());
                    Assert.assertFalse("Не указано описание курса " + courseUrl, driver.findElement(By.xpath("//h1[@class='course-header2__subtitle']")).getText().isEmpty());
                    commonCheck();
                    break;
                case "https://otus.ru/lessons/kotlin-qa-engineer/?int_source=courses_catalog&int_term=testing"://в этой карточке отсутствует характеристика Формат
                    Assert.assertFalse("Не указан заголовок курса " + courseUrl, driver.findElement(By.xpath("//div[@class='course-header2__title']")).getText().isEmpty());
                    Assert.assertFalse("Не указано описание курса " + courseUrl, driver.findElement(By.xpath("//div[@class='course-header2__admin-text']")).getText().isEmpty());
                    Assert.assertFalse("Не указана длительность курса " + courseUrl, driver.findElement(By.xpath("//p[@class='course-header2-bottom__item-text']")).getText().isEmpty());
